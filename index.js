...
app.put('/api/vehicle/:vehicleId/user/:userId', function(req, res, next) {
  db.update_ownership([Number(req.params.vehicleId),
  Number(req.params.userId)], function(err, resp) {
    if (err) {
      res.json(err);
    }
    res.status(200).json('Success!');
...
app.delete('/api/user/:userId/vehicle/:vehicleId', function(req, res, next) {
  db.destroy_ownership([Number(req.params.userId),
  Number(req.params.vehicleId)], function(err, resp) {
    if (err) {
      res.json(err);
    }
    res.status(200).json('Success!');
...
app.delete('/api/vehicle/:vehicleId', function(req, res, next) {
  db.destroy_vehicle(Number(req.params.vehicleId), function(error,
  response) {
    if (error) {
      res.json(err);
    }
